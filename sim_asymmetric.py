...
logger = logging.getLogger("sim_asymmetric")
...
def run_asymmetric_simulation():
    ...
    logger.info("Inizializzazione simulatore NetSquid per topologia asimmetrica...")
    ns.sim_reset()

    # 2. Costruzione del Setup Fisico
    config_path = os.path.join("config", "asymmetric_topology.json")
    ...
    metrics_collector = MetricsCollector(
        ...
        collection_name="asymmetric_metrics"
    )
    metrics_collector.start()

    # 5. Esecuzione e Salvataggio
    logger.info("Avvio della simulazione asimmetrica (durata totale: 1000 ms)...")
    ...
    if arrays:
        ...
        npz_path = os.path.join("results", "asymmetric_results.npz")
